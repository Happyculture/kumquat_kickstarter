 ...
 *   id = "full_view_mode_content"
 * )
 */
class FullViewModeContent extends ProcessPluginBase implements ContainerFactoryPluginInterface {
   ...
   */
  public function __construct(array $configuration, $plugin_id, $plugin_definition, FieldTypePluginManagerInterface $fieldTypePluginManager, FormatterPluginManager $formatterPluginManager, EntityFieldManagerInterface $entityFieldManager, MigrationInterface $migration = NULL) {
  ...
  public function transform($value, MigrateExecutableInterface $migrate_executable, Row $row, $destination_property) {
    if (empty($this->configuration['entity_type']) || empty($this->configuration['bundle'])) {
      throw new MigrateException('FullViewModeContent plugin is missing entity_type or bundle configuration.');
    }
    ...
    foreach ($sorted_fields as $type => $definitions) {
      foreach ($definitions as $field_name => $field_definition) {
        ...
        $field_formatter = $this->fieldTypes[$field_type]['default_formatter'];
        $content[$field_name] = [
          'type' => $field_formatter,
          'label' => 'hidden',
          'weight' => $weight++,
          'region' => 'content',
          'settings' => $this->formatterPluginManager->getDefaultSettings($field_formatter),
